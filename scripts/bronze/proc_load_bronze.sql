...
BEGIN
	DECLARE @load_bronze_start DATETIME, 
			@load_bronze_end DATETIME, 
			@start_time DATETIME, 
			@end_time DATETIME;

	BEGIN TRY
	SET @load_bronze_start = GETDATE();
		PRINT '======================================';
	...
	SET @load_bronze_end = GETDATE();
	PRINT '>> Complete Load Duration of Bronze Layer: ' + CAST(DATEDIFF(second, @load_bronze_start, @load_bronze_end) AS NVARCHAR) + ' seconds';
	END TRY
